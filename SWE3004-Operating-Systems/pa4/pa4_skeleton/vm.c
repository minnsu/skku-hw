...
// mem: kalloc가 반환하는 주소 그대로 넣을 것
void
lru_add(pde_t *pgdir, char *mem, char *va){
  // version 0 : physical address를 index로 -> 오류 발생시 v1 사용할 것
  int idx = ADDR2IDX(mem);
  // version 1 : 비어있는 첫번째 idx 사용
  // int idx = 0;
  // for(; idx < PHYSTOP/PGSIZE; idx++) {
  //   if(pages[idx] == 0)
  //     break;
  // }
  acquire(&page_lock);
