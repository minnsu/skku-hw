...
int append_path(const char *cmd, char *new_cmd){
    ...
    else if(!strcmp(cmd, "head") || !strcmp(cmd, "tail") || !strcmp(cmd, "cat")
        || !strcmp(cmd, "cp") || !strcmp(cmd, "mv") || !strcmp(cmd, "rm") || !strcmp(cmd, "pwd"))
...
void splitcmds(char *cmds, char *argv[]){
    // version 0 : strtok by space character
    // int idx = 0;
    ...
    if(i_red_count || o_red_count){
        ...
        for(int i = 0; i < MAX_ARGC && argv[i]; i++){
            if(!strcmp(argv[i], "<")){
                ...
                end_cmd = i < end_cmd ? i : end_cmd;
                write_flag |= (o_red_count == 1 ? O_TRUNC : O_APPEND);
                close(1);
