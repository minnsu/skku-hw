...
sfp float2sfp(float input){
    ...
    if(EXP_int - 127 < -14){
        FRAC_sfp = FRAC_sfp | 0x00000400;
        EXP_sfp = 0x0000 << 10; // 00000으로 설정
        FRAC_sfp = FRAC_sfp >> (-14-(EXP_int-127));
    }
...
float sfp2float(sfp input){
    ...
    if(EXP_int == 0){
        int i = 0;
        for(i = 0; !(0x00800000 & FRAC_new); i++){
            FRAC_new = FRAC_new << 1;
        }
        FRAC_new = FRAC_new & 0x007FFFFF; // 0000 0000 0111 1111 1111 1111 1111 1111
        EXP_new = (127 - 1 - i) << 23;
    }
