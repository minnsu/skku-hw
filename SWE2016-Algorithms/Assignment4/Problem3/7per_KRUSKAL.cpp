...
unsigned long long MST_kruskal(vector<vector<pair<int, int>>> &graph, int &n, multiset<pair<pair<int, int>, int>, compare> &unused, vector<vector<pair<int, int>>> &mst){
	multiset<pair<pair<int, int>, int>, compare> edges;
	...
	for(int i = 0; i < n; i++){
		...
		for(int j = 0; j < graph[i].size(); j++){
			if(i < graph[i][j].first){
				edges.insert(make_pair(make_pair(i, graph[i][j].first), graph[i][j].second));
				// {{ src, dest }, cost }
			}
		}
	}
...
int main(int argc, char** argv){
	...
	multiset<pair<pair<int, int>, int>, compare> unused;
	vector<vector<pair<int, int>>> mst(n);

	unsigned long long answer = MST_kruskal(graph, n, unused, mst);
	if(answer == 0){
		fprintf(fw, "-1");
		return 0;
	}
