...
public class BTreeIndex implements Index {
   ...
   // minsu defined
   private BTreeDir parent;
   private BTreeLeaf left;
   private BTreeLeaf right;
   ...
   public void beforeFirst(Constant searchkey) {
      ...
      if(root.getLeft() == -1)
         ...
         BlockId leftblk = new BlockId(leaftbl, root.getLeft());
         left = new BTreeLeaf(tx, leftblk, leafLayout, searchkey);
      }
      if(root.getRight() == -1)
         right = null;
      else {
         BlockId rightblk = new BlockId(leaftbl, root.getRight());
         right = new BTreeLeaf(tx, rightblk, leafLayout, searchkey);
      }
   ...
   public void insert(Constant dataval, RID datarid) {
      beforeFirst(dataval);
      DirEntry e = leaf.insert(datarid, left, right, parent);
      leaf.close();
      if (e == null)
         return;
      BTreeDir root = new BTreeDir(tx, rootblk, dirLayout);
