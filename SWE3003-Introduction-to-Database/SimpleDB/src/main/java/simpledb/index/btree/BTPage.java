...
public class BTPage {
   ...
   // minsu defined 
   public void setSlotKey(int slot, Constant searchkey, int blknum) {
      delete(slot);
      insertDir(slot, searchkey, blknum);
   }
   ...
   // minsu defined methods
   public void moveToDest(int srcSlot, int destSlot, BTPage dest){
      Constant val = this.getDataVal(srcSlot);
      RID rid = this.getDataRid(srcSlot);
      delete(srcSlot);
      dest.insertLeaf(destSlot, val, rid);
   }
