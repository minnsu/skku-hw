...
public class BTreeDir {
   ...
   private BlockId findChildBlock(Constant searchkey) {
      ...
      // minsu defined start
      if(slot > 0)
         this.left = contents.getChildNum(slot - 1);
      else
         this.left = -1;
      if(contents.getNumRecs() - 1 > slot)
         this.right = -1;
      else
         this.right = contents.getChildNum(slot + 1);
      // minsu defined end
   ...
   public BTreeDir getParent() {
      return this;
   }
   public void setSlotKey(int slot, Constant searchkey, int blknum) {
      this.contents.setSlotKey(slot, searchkey, blknum);
   }
}
